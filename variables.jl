...
@variable(
    model,
    PrcAct[REGION, YEAR, MILEYR, PROCESS, TSLICE] >= 0,
    container = SparseAxisArray
)
@variable(
    model,
    PrcFlo[REGION, YEAR, MILEYR, PROCESS, COMMTY, TSLICE] >= 0,
    container = SparseAxisArray
)
@variable(
    model,
    IreFlo[REGION, YEAR, MILEYR, PROCESS, COMMTY, TSLICE, IMPEXP] >= 0,
    container = SparseAxisArray
)
@variable(
    model,
    StgFlo[REGION, YEAR, MILEYR, PROCESS, COMMTY, TSLICE, INOUT] >= 0,
    container = SparseAxisArray
)
